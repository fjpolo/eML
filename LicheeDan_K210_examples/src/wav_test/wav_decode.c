...
enum errorcode_e wav_init(struct wav_file_t *wav_file)
{
	...
	memcpy(wav_head_buff, wav_file->wave_file, 512);
	...
	index += 4;
	if ((LG_READ_WORD(index) + 8) != wav_file->wave_file_len)
	...
	wav_file->wave_file_curpos = index;
...
enum errorcode_e wav_decode_init(struct wav_file_t *wav_file)
{
	...
	tmp = wav_file->wave_file_len - wav_file->wave_file_curpos;
	wav_file->buff0_read_len = wav_file->buff0_len > tmp ? tmp : wav_file->buff0_len;
	memcpy(wav_file->buff0, (wav_file->wave_file + wav_file->wave_file_curpos), wav_file->buff0_read_len);
	wav_file->wave_file_curpos += wav_file->buff0_read_len;
...
enum errorcode_e wav_decode(struct wav_file_t *wav_file)
{
	...
	if (wav_file->buff0_used == 0)
	{
		...
		if (wav_file->buff0_len > wav_file->buff0_read_len)
		{
			if (wav_file->wave_file_curpos == wav_file->wave_file_len)
				return FILE_END;
			return FILE_FAIL;
		}
	}
	...
	{
		tmp = wav_file->wave_file_len - wav_file->wave_file_curpos;
		wav_file->buff1_read_len = (wav_file->buff1_len > tmp) ? tmp : wav_file->buff1_len;
		memcpy(wav_file->buff1, wav_file->wave_file + wav_file->wave_file_curpos, wav_file->buff1_read_len);
		wav_file->wave_file_curpos += wav_file->buff0_read_len;
