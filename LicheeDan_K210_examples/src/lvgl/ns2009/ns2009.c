...
{
    uint8_t ret, buf[2];
    ret = i2c_recv_data(0, &cmd, 1, buf, 2);
    if (ret != 0)
        return 0;
    if (val)
        *val = (buf[0] << 4) | (buf[1] >> 4);
...
struct ts_ns2009_pdata_t *ts_ns2009_probe(void)
{
    ...
    //FIXME 需要放到外边
    i2c_init(0, NS2009_SLV_ADDR, 7, 100000);
    ...
    ts_ns2009_pdata = malloc(sizeof(struct ts_ns2009_pdata_t));
    if (!ts_ns2009_pdata)
    {
        return NULL;
    }
    ...
    tsfilter_setcal(ts_ns2009_pdata->filter, &cal[0]);
    //
    ts_ns2009_pdata->x = 0;
    ts_ns2009_pdata->y = 0;
    ts_ns2009_pdata->press = 0;
    //
    ts_ns2009_pdata->event = malloc(sizeof(struct ts_ns2009_event_t));
...
void ts_ns2009_remove(struct ts_ns2009_pdata_t *ts_ns2009_pdata)
{
    ...
    if (ts_ns2009_pdata->event)
        free(ts_ns2009_pdata->event);

    if (ts_ns2009_pdata)
        free(ts_ns2009_pdata);
}
